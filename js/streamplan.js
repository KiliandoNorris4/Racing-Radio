    ...
    return `
        ...
        <div class="${classes}">
            ...
            <div class="stream-time">
                ...
                🕒 ${stream.time ? stream.time.slice(0,5) + " Uhr" : "Streamstart ungeplant"}
...
function getNextStream() {
    ...
    const upcoming = streams
        .filter(stream => {
            ...
            if (!stream.time) {
                return new Date(stream.date) >= new Date(now.toDateString());
            }
        ...
        .sort((a, b) => {
            ...
            const aDate = new Date(`${a.date}T${a.time || "23:59"}`);
            const bDate = new Date(`${b.date}T${b.time || "23:59"}`);
